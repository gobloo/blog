---
layout: post
title: DKIM - A simple explication
date: 01-11-2025
categories: [Notes]
tag: [Red Team, Blue Team, Phishing, Email Analysis]
---

# Understanding DKIM (DomainKeys Identified Mail) for Email Security
...
4. **Verification:** The receiver’s email server extracts the DKIM-Signature, retrieves your public key from DNS under a specific selector, and checks if the signature matches the email content.
5. **Authentication Check:** If the signature matches, it confirms the email is genuine and unchanged during transit. The signing domain must match the domain in the email’s From address to prevent spoofing.
    ![DKIM Workflow Diagram](image-1.png)
...
Creating a DKIM record involves generating a cryptographic key pair and publishing the public key in your domain’s DNS as a TXT record.
...
- **TXT Record:** Stores the actual DKIM public key in your domain’s DNS.
- **CNAME Record:** Points to a key managed by a third-party provider, useful for easy key rotation.

Always follow your email provider’s instructions to add DKIM DNS records properly and verify they are working.

---

> Using DKIM helps ensure your emails are trusted and are less likely to be marked as spam or spoofed.
